# -*- coding: utf-8 -*-
"""Bagging_tekrar_14_08_2025.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1lTWTtlPhM41J5DPBdxzuyd0eM_T2c2ct
...
def Bagging_Model(dataframe, target,  test_size=0.20, cv=10,results=False,save_model=False):
  ...
  if results:
    ...
    rmse_train = root_mean_squared_error(y_train, bag_model.predict(X_train))
    rmse_test = root_mean_squared_error(y_test, bag_model.predict(X_test))
    ...
    r2_train = r2_score(y_train, bag_model.predict(X_train))
    r2_test = r2_score(y_test, bag_model.predict(X_test))
    cv_results_mse = cross_validate(bag_model, X, y, cv=cv, scoring="neg_mean_squared_error")
    cv_results_rmse = cross_validate(bag_model, X, y, cv=cv, scoring="neg_root_mean_squared_error")
...
bag_model = Bagging_Model(df, "Salary",  test_size=0.20, cv=10,results=True,save_model=True)

"""# Load a Model and The Prediction"""

def load_model(pklfile):
  model_disc = joblib.load(pklfile)
  return model_disc

model_disc = load_model("bag_model.pkl")
...
def Bagging_Model_Tuning(dataframe, target,  test_size=0.20, cv=10,results=False,save_model=False):
  X = dataframe.drop(target, axis=1)
  y = dataframe[target]
  X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=1)
  bag_model = BaggingRegressor(bootstrap_features=True)
  bag_params = {
      "max_features" : [3,5,7,9,12,15,"auto"],
      "n_estimators" : range(2,50),
  }
  bag_cv_model = GridSearchCV(bag_model, bag_params, cv=cv, verbose=True).fit(X_train, y_train)
  bag_modle_tuned = bag_model.set_params(**bag_cv_model.best_params_, random_state=1).fit(X,y)

  if results:
    mse_train = mean_squared_error(y_train, bag_modle_tuned.predict(X_train))
    mse_test = mean_squared_error(y_test, bag_modle_tuned.predict(X_test))
    rmse_train = root_mean_squared_error(y_train, bag_modle_tuned.predict(X_train))
    rmse_test = root_mean_squared_error(y_test, bag_modle_tuned.predict(X_test))
    mae_train = mean_absolute_error(y_train, bag_modle_tuned.predict(X_train))
    mae_test = mean_absolute_error(y_test, bag_modle_tuned.predict(X_test))
    r2_train = r2_score(y_train, bag_modle_tuned.predict(X_train))
    r2_test = r2_score(y_test, bag_modle_tuned.predict(X_test))
    cv_results_mse = cross_validate(bag_model, X, y, cv=cv, scoring="neg_mean_squared_error")
    cv_results_rmse = cross_validate(bag_model, X, y, cv=cv, scoring="neg_root_mean_squared_error")
  ...
  if save_model:
    joblib.dump(bag_modle_tuned, "bag_modle_tuned.pkl")
    print("Model Saved...")

  return bag_modle_tuned

bag_modle_tuned = Bagging_Model_Tuning(df, "Salary",  test_size=0.20, cv=10,results=True,save_model=True)
...
model_disc = load_model("bag_modle_tuned.pkl")
