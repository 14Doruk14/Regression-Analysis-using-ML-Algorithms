# -*- coding: utf-8 -*-
"""Adaboost_Regressor_19_8_2025_pythontekrar.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14nMJCPPhURxF4FGbCkL0iAz4ccqfeQbQ
...
def Adaboost_Model(dataframe, target,  test_size=0.20, cv=10, results=False,save_model=False, plot_importance=False):
  X = dataframe.drop(target, axis=1)
  y = dataframe[target]
  X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=1)
  adaboost_model = AdaBoostRegressor(random_state=1).fit(X_train, y_train)

  if results:
    mse_train = mean_squared_error(y_train, adaboost_model.predict(X_train))
    mse_test = mean_squared_error(y_test, adaboost_model.predict(X_test))
    rmse_train = root_mean_squared_error(y_train, adaboost_model.predict(X_train))
    rmse_test = root_mean_squared_error(y_test, adaboost_model.predict(X_test))
    mae_train = mean_absolute_error(y_train, adaboost_model.predict(X_train))
    mae_test = mean_absolute_error(y_test, adaboost_model.predict(X_test))
    r2_train = r2_score(y_train, adaboost_model.predict(X_train))
    r2_test = r2_score(y_test, adaboost_model.predict(X_test))
    cv_result_mse = cross_validate(adaboost_model, X,y, cv=cv, scoring="neg_mean_squared_error")
    cv_result_rmse = cross_validate(adaboost_model, X, y, cv=cv, scoring="neg_root_mean_squared_error")
  ...
  if plot_importance:
    feature_imp = pd.DataFrame({'Value': adaboost_model.feature_importances_, 'Feature': X.columns}).sort_values(by="Value", ascending=False)
...
adaboost_model = Adaboost_Model(df, "Salary",  test_size=0.20, cv=10, results=True,save_model=True, plot_importance=True)
...
model_disc = laod_model("adaboost_model.pkl")
...
def AdaBoost_Model_Tuning(dataframe, target,  test_size=0.20, cv=10, results=False,save_model=False, plot_importance=False):
  X = dataframe.drop(target, axis=1)
  y = dataframe[target]
  X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=1)
  ...
  adaboost_cv_model = GridSearchCV(adaboost_model, adaboost_params, cv=cv, verbose=True, n_jobs=-1).fit(X_train, y_train)
  adaboost_model_tuned = adaboost_model.set_params(**adaboost_cv_model.best_params_, random_state=1).fit(X,y)

  if results:
    mse_train = mean_squared_error(y_train, adaboost_model_tuned.predict(X_train))
    mse_test = mean_squared_error(y_test, adaboost_model_tuned.predict(X_test))
    rmse_train = root_mean_squared_error(y_train, adaboost_model_tuned.predict(X_train))
    rmse_test = root_mean_squared_error(y_test, adaboost_model_tuned.predict(X_test))
    mae_train = mean_absolute_error(y_train, adaboost_model_tuned.predict(X_train))
    mae_test = mean_absolute_error(y_test, adaboost_model_tuned.predict(X_test))
    r2_train = r2_score(y_train, adaboost_model_tuned.predict(X_train))
    r2_test = r2_score(y_test, adaboost_model.predict(X_test))
    cv_result_mse = cross_validate(adaboost_model, X,y, cv=cv, scoring="neg_mean_squared_error")
    cv_result_rmse = cross_validate(adaboost_model, X, y, cv=cv, scoring="neg_root_mean_squared_error")
    print("Best Params: ", adaboost_cv_model.best_params_)
    print("MSE Train: ", "%.3f" % mse_train)
    print("MSE Test: ", "%.3f" % mse_test)
    print("RMSE Train: ", "%.3f" % rmse_train)
    print("RMSE Test: ", "%.3f" % rmse_test)
  ...
  if save_model:
    joblib.dump(adaboost_model, "adaboost_model_tuned.pkl")

  if plot_importance:
    feature_imp = pd.DataFrame({'Value': adaboost_model_tuned.feature_importances_, 'Feature': X.columns}).sort_values(by="Value", ascending=False)
    plt.figure(figsize=(8,6))
    sns.barplot(x="Value", y="Feature", data=feature_imp)
    plt.title("Feature Importance")
    plt.tight_layout()
  ...
  return adaboost_model_tuned

adaboost_model_tuned = AdaBoost_Model_Tuning(df, "Salary",  test_size=0.20, cv=10, results=True,save_model=True, plot_importance=True)
...
model_disc = laod_model("adaboost_model_tuned.pkl")
...
feature_imp = plot_importance(adaboost_model_tuned, X,save=True)
