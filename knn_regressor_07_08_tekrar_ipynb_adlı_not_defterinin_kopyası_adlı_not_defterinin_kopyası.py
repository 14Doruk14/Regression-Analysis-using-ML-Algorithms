# -*- coding: utf-8 -*-
"""KNN_Regressor_07_08_tekrar.ipynb adlı not defterinin kopyası adlı not defterinin kopyası

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1lBsHmh96H0HDKGFMRyTRNvGVz17lEJUR
...
def KNN_Regressor(dataframe, target,  test_size=0.20, results=False,save_model=False):
  ...
  if results:
    ...
    rmse_train = root_mean_squared_error(y_train, knn_regressor_model.predict(X_train))
    rmse_test = root_mean_squared_error(y_test, knn_regressor_model.predict(X_test))
  ...
  if save_model:
    joblib.dump(knn_regressor_model, "knn_regressor_model.pkl")

  return knn_regressor_model

knn_regressor_model = KNN_Regressor(df, "Salary",  test_size=0.20, results=True,save_model=True)
...
new_data_predict(new_data, "knn_regressor_model.pkl")
...
new_data_predict2(new_player_without_target, "knn_regressor_model.pkl")

"""# KNN Regressor Model Tuning"""

def KNN_Regressor_Tuning(dataframe, target, test_size=0.20, k=30, cv=10, results=False, save_model=False,iteration_report= False):
  X = dataframe.drop(target, axis=1)
  y = dataframe[target]
  X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=1)
  knn_params = {'n_neighbors' : np.arange(1, k, 1)}
  knn_regressor_model = KNeighborsRegressor()
  knn_cv_regressor_model = GridSearchCV(knn_regressor_model, knn_params, cv=cv, verbose=1, n_jobs=-1).fit(X_train, y_train)
  ...
  if results:
    ...
    rmse_train = root_mean_squared_error(y_train, knn_regressor_tuned.predict(X_train))
    rmse_test = root_mean_squared_error(y_test, knn_regressor_tuned.predict(X_test))
    ...
    print(f"K Best Parmas: {knn_cv_regressor_model.best_params_}")
    print("MSE Train: ", "%.3f" % mse_train)
    print("MSE Test: ", "%.3f" % mse_test)
    print("RMSE Train: ", "%.3f" % rmse_train)
    print("RMSE Test: ", "%.3f" % rmse_test)
  ...
  if iteration_report:
    ...
    for k_num in range(k):
      k_num = k_num + 1
      print(30*"#", "Iteration Report", 30*"#")
      knn_regressor_model = KNeighborsRegressor(n_neighbors=k_num).fit(X_train, y_train)
      rmse = root_mean_squared_error(y_train, knn_regressor_model.predict(X_train))
      rmse_cv = -cross_val_score(knn_regressor_model, X_train, y_train, cv=cv, scoring="neg_root_mean_squared_error").mean()
      RMSE.append(rmse)
      RMSE_CV.append(rmse_cv)
      print(f"K = {k_num}, RMSE = {round(rmse, 3)} RMSE_CV = {round(rmse_cv, 3)}")



  if save_model:
    joblib.dump(knn_regressor_tuned, "knn_regressor_tuned.pkl")
    print("Model Tuned Saved...")

  return knn_regressor_tuned

knn_regressor_tuned = KNN_Regressor_Tuning(df, "Salary", test_size=0.20, k=30, cv=10, results=True, save_model=True, iteration_report=True)

new_data_predict2(new_player_without_target, "knn_regressor_tuned.pkl")
